        ...
        <div className="p-2 sm:p-4">
          <div className="flex items-start gap-1.5 sm:gap-3">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1">
              <button
                onClick={() => onUpdate({ reminderEnabled: !task.reminderEnabled })}
                className={`p-1 sm:p-1.5 rounded-lg transition-colors ${
                  task.reminderEnabled
                    ? "text-amber-500 bg-amber-50 hover:bg-amber-100"
                    : "text-text-muted hover:text-amber-500 hover:bg-amber-50"
                }`}
                title={task.reminderEnabled ? "Reminder on — click to remove" : "Set daily reminder"}
              >
                <svg className="w-4 h-4 sm:w-5 sm:h-5" fill={task.reminderEnabled ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
