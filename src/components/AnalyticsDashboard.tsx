  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-3 sm:gap-4">
        {/* Circular Progress */}
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-2 sm:mb-4 text-sm sm:text-base">Overall Progress</h3>
          <div className="flex items-center justify-center">
            <div className="relative w-28 h-28 sm:w-40 sm:h-40">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-xl sm:text-3xl font-bold text-text-primary">
                  {Math.round(analytics.completionRate)}%
                </span>
                <span className="text-xs sm:text-sm text-text-muted">Complete</span>
              </div>
            </div>
          </div>
          <div className="mt-2 sm:mt-4 grid grid-cols-2 gap-2 sm:gap-4 text-center">
            <div>
              ...
              <p className="text-xs sm:text-sm text-text-muted">Tasks Done</p>
            </div>
            <div>
              <p className="text-lg sm:text-2xl font-bold text-text-muted">{analytics.totalTasks - analytics.completedTasks}</p>
              <p className="text-xs sm:text-sm text-text-muted">Remaining</p>
            </div>
          </div>
        </div>
        ...
        {/* Projection Card */}
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-2 sm:mb-4 text-sm sm:text-base">Projected Completion</h3>
          ...
          {analytics.velocityTrend.length > 1 && (
            <div className="mt-4 pt-4 border-t border-border">
              <p className="text-sm font-medium text-text-secondary mb-2">Velocity Trend</p>
              ...
              <p className="text-xs text-text-muted mt-1 text-center">Last 14 active days</p>
            </div>
          )}
        </div>
      </div>
      ...
      {analytics.weeklyProgress.length > 0 && (
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-2 sm:mb-4 text-sm sm:text-base">Weekly Progress</h3>
          <div className="space-y-2 sm:space-y-3">
            {analytics.weeklyProgress.slice(-8).map((week) => (
              <div key={week.weekStart} className="flex items-center gap-2 sm:gap-4">
                <div className="w-14 sm:w-24 text-[10px] sm:text-xs text-text-muted flex-shrink-0">
                  {new Date(week.weekStart).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="h-4 sm:h-6 bg-surface-hover rounded-full overflow-hidden flex">
                    <div
      ...
      {analytics.costByPhase.length > 0 && (
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-2 sm:mb-4 text-sm sm:text-base">Budget by Phase</h3>
          <div className="space-y-4">
            {analytics.costByPhase.map((phase) => (
              <div key={phase.phase}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium text-text-secondary">{phase.phase}</span>
          ...
          <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-border grid grid-cols-3 gap-2 sm:gap-4 text-center">
            <div>
              <p className="text-sm sm:text-lg font-bold text-text-primary truncate">{formatCurrency(analytics.totalEstimatedCost)}</p>
              <p className="text-[10px] sm:text-xs text-text-muted">Estimated</p>
            </div>
            <div>
              <p className="text-sm sm:text-lg font-bold text-emerald-600 truncate">{formatCurrency(analytics.totalActualCost)}</p>
              <p className="text-[10px] sm:text-xs text-text-muted">Spent</p>
            </div>
            <div>
      ...
      {analytics.totalSubsteps > 0 && (
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-2 sm:mb-4 text-sm sm:text-base">Substeps Overview</h3>
          <div className="flex items-center gap-3 sm:gap-4 sm:p-6">
            <div className="flex-1 min-w-0">
              <div className="h-3 sm:h-4 bg-surface-hover rounded-full overflow-hidden">
                <div
            ...
            <div className="text-right flex-shrink-0">
              ...
              <p className="text-[10px] sm:text-xs text-text-muted">Substeps Complete</p>
            </div>
          </div>
        </div>
      )}
      ...
      {analytics.goalBreakdown && analytics.goalBreakdown.length > 0 && (
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-3 sm:mb-4 text-sm sm:text-base">Goal Breakdown</h3>
          <div className="space-y-3">
            {analytics.goalBreakdown.map((goal) => (
              <div key={goal.goalId} className="border-b border-border pb-3 last:border-0 last:pb-0">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs sm:text-sm font-semibold text-text-secondary truncate flex-1">{goal.title}</h4>
      ...
      {analytics.phaseCompletion && analytics.phaseCompletion.length > 0 && (
        <div className="bg-surface rounded-xl sm:rounded-2xl shadow-lg border border-border p-3 sm:p-6">
          <h3 className="font-bold text-text-primary mb-3 sm:mb-4 text-sm sm:text-base">Phase Completion</h3>
          <div className="space-y-3">
            {analytics.phaseCompletion.map((phase) => (
              <div key={phase.phaseId} className="border-b border-border pb-3 last:border-0 last:pb-0">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs sm:text-sm font-semibold text-text-secondary truncate flex-1">{phase.name}</h4>
...
function MetricCard({ title, value, subtitle, icon, color }: MetricCardProps) {
  ...
  return (
    <div className="bg-surface rounded-lg sm:rounded-xl shadow-lg border border-border p-2.5 sm:p-4 overflow-hidden relative">
      ...
      <p className="text-sm sm:text-2xl font-bold text-text-primary truncate">{value}</p>
      <p className="text-[10px] sm:text-xs text-text-muted mt-0.5 sm:mt-1">{title}</p>
      <p className="text-[10px] sm:text-xs text-text-muted truncate">{subtitle}</p>
    </div>
  );
}
