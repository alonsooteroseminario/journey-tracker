  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          onClick={() => onUpdate({ reminderEnabled: !substep.reminderEnabled })}
          className={`p-1 rounded transition-colors ${
            substep.reminderEnabled
              ? "text-amber-500 bg-amber-50 hover:bg-amber-100"
              : "text-text-muted hover:text-amber-500 hover:bg-amber-50"
          }`}
          title={substep.reminderEnabled ? "Reminder on — click to remove" : "Set daily reminder"}
        >
          <svg className="w-4 h-4" fill={substep.reminderEnabled ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
